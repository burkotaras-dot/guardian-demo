...
3. **Перевіряти в прев'ю ЛИШЕ коли в цій відповіді реально був `Edit`/`Write` по `index.html`
   з видимим (візуальним/поведінковим) наслідком.** Тоді — світла і темна тема, **виміряні
   значення** (`getComputedStyle` / `getBoundingClientRect`), а не «має працювати», і ніколи не
   просити користувача перевірити самому.
   **НЕ ганяти прев'ю**, коли: код не змінювався (підтвердження, відповідь на питання,
   git-операція, робота з пам'яттю/`.md`), або правка суто текстова/несуттєва без візуального ефекту.
   **Режим «просто зроби»:** якщо користувач каже «зроби без прев'ю» / «не перевіряй» — застосувати
   правку й зупинитись; перевірити пакетом наприкінці за окремим проханням, а не після кожного руху.
