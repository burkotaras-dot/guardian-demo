...
## ⚠️ Цей документ відповідає лише на питання «як це **виглядає**»

Поведінка — коли система питає, коли повідомляє, як називає дії, що робить при помилці —
описана в окремому документі **[`GUARDIAN-INTERACTION-STANDARDS.md`](GUARDIAN-INTERACTION-STANDARDS.md)**.

17.08.2026 поведінкові розділи (колишні §5 States, §6.1–6.4, §10 Terminology, §11 Interaction flows)
**перенесені туди** й тут видалені. Дублювання правил у двох документах — головна причина, чому
стандарти розходяться, тому кожне правило живе рівно в одному місці. Карта «стара адреса → нова» —
у розділі «Карта зведення» нового документа.

## Зміст
1. [Navigation](#1-navigation) — ✅ overlay modal · back · close · steppers · frameless
2. [Page layout](#2-page-layout) — ✅ dark canvas
3. [Forms](#3-forms) — ✅ dropdown hover + selected state
4. [Tables](#4-tables) — ✅ table headers
5. States — ➡️ перенесено в Interaction Standards §8–§14
6. [Buttons](#6-buttons) — ✅ 6.1 канонічний footer модалки · **6.5 канонічний набір з Figma** · 6.6 сегменти й таби
7. Cards — ⏳
8. KPI — ⏳
9. Spacing — ⏳
10. Terminology — ➡️ перенесено в Interaction Standards §10
11. Interaction flows — ➡️ перенесено в Interaction Standards §2, §3.3, §3.4, §6.3
...
14. [Модальне вікно — канонічна оболонка](#14-модальне-вікно--канонічна-оболонка) — ✅ **база ВСІХ модалок** · header · stepper · body-слот · footer
15. [Progress bar](#15-progress-bar) — ✅ **кольори з Figma** для світлої й темної теми
...
### 6.1 Канонічний footer модального вікна

**Source of truth:** Figma `eXz33Qu7v58JjOiatTptAE`, component set `3023:5088`.
Footer — частина канонічної оболонки модалки; header, stepper і body-слот описані в **§14**.
...
**Rule.** В застосунку існує **рівно шість** типів кнопок: `Primary`, `Secondary`, `Text button`,
`Third`, `Back`, `Only icon`. Створюючи будь-що нове, брати кнопку **звідси** — не вигадувати власну
й не копіювати старі inline-стилі. Джерело: Figma «Guardian UI Design System based on Mantine — v5.10»,
секція `Buttons` (fileKey `eXz33Qu7v58JjOiatTptAE`, node `2988:70531`). Оновлено з макета 2026-08-17
(**5-та ревізія** — доданий `Only icon`).
...
#### Only icon — НЕЙТРАЛЬНИЙ квадрат з гліфом (додано 17.08.2026, `3204:13731`)

Шостий тип і **єдиний нейтральний** у наборі — він закриває дефект 8 (див. нижче). Квадрат
`32×32` з гліфом `16×16` усередині, без тексту. Стани: Default / Hover / Disable × Light / Dark.
Стану `loading` немає.

**Геометрія — однакова в усіх шести станах:** `32×32` · `border-radius:8px` · `border:1px solid` ·
`padding:6px` · гліф `16×16`, `fill:none` + `stroke:currentColor`.

⚠️ **Radius `8px`, а не `10px`** — це єдиний тип, що виходить із загального правила `radius:10px`.
Свідомо: у квадраті 32×32 радіус 10 зʼїдає кут сильніше, ніж у прямокутної кнопки.
Не «виправляти» на 10 — тоді `Only icon` перестане збігатися з макетом.

| Тема | Стан | Фон | Бордер | Гліф |
|---|---|---|---|---|
| Light | default | `#FFFFFF` | `#E2E8F0` | `#64748B` |
| Light | hover | `#F3FAF6` | `rgba(16,185,129,0.32)` | `#0F172A` |
| Light | disable | `#F8FAFC` | `#E2E8F0` | `#64748B` + `opacity:0.4` |
| Dark | default | `rgba(255,255,255,0.05)` | `#04363A` | `rgba(255,255,255,0.7)` |
| Dark | hover | `rgba(255,255,255,0.05)` | `rgba(16,185,129,0.30)` | `#FFFFFF` |
| Dark | disable | `rgba(255,255,255,0.05)` | `#04363A` | `rgba(255,255,255,0.7)` + `opacity:0.4` |

**Збіг із нашими токенами (перевірено):** dark-фон `rgba(255,255,255,0.05)` — це рівно канонічна
заливка інпутів у dark, а `#04363A` = `--border-default` (`--p-dark-border`). Light `#E2E8F0` =
`--border`, `#F8FAFC` = `--bg-canvas`, `#0F172A` = `--text-default`, `#64748B` = `--text-muted`
у dark-палітрі. Тобто тип побудований на **чинній нейтральній хромі застосунку**, а не на новій.

**ПРАВИЛО ФОНУ: кнопка ніколи не повторює поверхню під собою (18.08.2026, user).**
Light default = **`#FFFFFF`** (`var(--bg-surface)`) — так у таблиці вище, і так тепер у коді.
Причина: тулбари в застосунку лежать на полотні `--bg-canvas` `#F8FAFC`, і сірий чип на сірому
полотні зливався — кнопка читалась лише як контур. На **білій картці** роль розрізнення бере
1px бордер `--border`, а не заливка — тому окремого «чипового» варіанта не існує.
Це та сама логіка, що вже діє для `.btn-outline` (завжди білий на light).
**Dark без змін:** `rgba(255,255,255,0.05)` уже світліший і за темне полотно, і за темну картку.
Імплементація: `.btn-icon { background:var(--bg-surface) }` (`index.html:571`).
Історія: до 18.08 базова заливка була `#F8F9FA`; вирівняно на макет, скасовано разом із тимчасовим
контекстним класом `.btn-icon-canvas` (проіснував один крок, у коді його немає).

**Іконки** (окремий фрейм `3193:18185`, усі `16×16`): `Set as baseline` `3193:18184` ·
`Fix now` `3193:18219` · `View` `3193:18220` · `Compare` `3193:18224` (у макеті помилка в назві —
`Camoare`) · `Play` `3193:18225` · `Consensus Comparison` `3193:18226` · `Search` `3193:18228` ·
`Edit` `3193:18236` · `More` `3193:18237` · `Scanning` `3193:18311` · `Mark as Unmanaged` `3193:21686`.
Гліфи тягнути з макета при імплементації — не перемальовувати від руки.

**Коли що (три квадрати 32×32 в наборі — не плутати):**
`Back` = повернення, зелена хрома, лише стрілка · `Third` = повернення з підписом ЗЗОВНІ квадрата ·
`Only icon` = **дія над обʼєктом** (View / Edit / More / Compare) — нейтральна хрома, підпису немає.
...
Токени, додані 5-ю ревізією (усі — для `Only icon`, поза родиною `new buttons/*`):
`Primitive/Neutral/500` `#64748B` (гліф default) · `Semantic/Text/Default` `#0F172A` (гліф hover
light) · `Icon/White` `#FFFFFF` (гліф hover dark) · `Primitive/Radius/8` = `8`. Значення решти
пʼяти типів у 5-й ревізії **не змінились** — звірено за повним списком змінних секції.
...
8. ~~**У наборі немає нейтрального типу.**~~ **ЗАКРИТО 5-ю ревізією** — `Only icon` нейтральний.
   Але закриває він лише кнопки **без тексту**: `Cancel` / `Skip` / `Close` — це кнопки З ТЕКСТОМ,
   і нейтрального ТЕКСТОВОГО типу в наборі досі немає, тому `.btn-outline` лишається поза каноном.
   Питання до дизайнера звужується: **потрібен нейтральний варіант `Secondary`**.

Дефекти, знайдені в `Only icon` (5-та ревізія):

9. **`Disable` затемнено двома різними способами.** У light `opacity:0.4` стоїть на самому гліфі
   (фон і бордер лишаються на повну), у dark — на всій кнопці. Решта пʼяти типів роблять
   `default + opacity:0.4` на кнопці. Звести до одного — на кнопці.
10. **Light `Disable` носить чужий auto-layout:** `padding:0 14px` + `gap:4px` (значення текстової
    кнопки) замість `padding:6px`, як у Default/Hover. Візуально не видно, бо розмір зафіксований
    `32×32`, але специфікація суперечить сама собі. Там же radius привʼязаний до
    `Primitive/Radius/8`, а в решти станів — до безіменного `8`.
11. **Дві родини зеленого в одному наборі.** Hover `Only icon` — `rgba(16,185,129,0.32)` (light) /
    `rgba(16,185,129,0.30)` (dark), тобто **стара** emerald-палітра §6.4, тоді як пʼять інших типів
    переведені на teal-родину `#00AF73`. Плюс самі `0.32` vs `0.30` — різні альфи для одного стану.
12. **Light hover змінює і фон, і бордер, dark hover — лише бордер** (фон у dark однаковий у всіх
    трьох станах). Через це в dark ховер помітно слабший.
13. Light `default` зібраний зі **стека трьох заливок** (біла поверх `#F8FAFC` поверх `#F8FAFC`) —
    артефакт шарів; ефективний колір `#FFFFFF`.
14. Ім'я іконки `Camoare` замість `Compare` (`3193:18224`).

**Додано в 5-й ревізії (17.08.2026):** тип `Only icon` — нейтральний квадрат `32×32` з гліфом
`16×16` і власний фрейм із 11 гліфів. Значення пʼяти наявних типів не змінились.
...
> **Критерій вибору** між табами й сегментом — поведінковий, тому канонічно він живе в
> Interaction Standards §4.2. Нижче — та сама відповідь плюс візуальні значення, яких там немає.
...
## 14. Модальне вікно — КАНОНІЧНА ОБОЛОНКА

**Source of truth:** Figma `eXz33Qu7v58JjOiatTptAE`, компонент **`View of a Modular Window`**
`3023:4788` (варіанти `White mode` `3006:2134` / `Dark mode` `3023:4789`).

**Rule.** Це **база всіх без винятку модальних вікон** застосунку. Оболонка — header, stepper,
body-слот і footer — щоразу та сама; відрізняється **лише вміст слота**. Нове модальне вікно
ніколи не будується з нуля: береться `openModal()`, і в нього передається `bodyHtml`.
Якщо для нового вікна «не вистачає» оболонки — це привід уточнити канон, а не зверстати власну.

### 14.1 Оболонка

| Властивість | Значення |
|---|---|
| Ширина | `600px` (у коді `.gm-lg { max-width:600px }`) |
| Radius | `12px` |
| Shadow | `0 12px 40px rgba(0,0,0,0.3)` |
| Фон Light | `#FFFFFF` |
| Фон Dark | `#062528` |
| Overflow | `hidden` (щоб кути різали вміст) |

### 14.2 Header

`padding:22px 24px 10px` · `display:flex` · `align-items:flex-start` · `gap:12px`.
Тайтл і сабтайтл — в одній колонці зліва, кнопка Close — окремим елементом справа.

| Елемент | Шрифт | Light | Dark |
|---|---|---|---|
| **Title** | DM Sans **600 / 15px**, line-height `19.5px` | `#0F172A` | `#F8FAFC` |
| **Subtitle** | DM Sans **400 / 12px**, line-height `18px`, `padding-top:4px` | `#475569` | `#CBD5E1` |

Обидва кольори сабтайтла — це рівно токен **`--text-sec`** (light `#475569`, dark `#CBD5E1`).
Сабтайтл **не** `--text-muted`.

**Close:** `28×28`, `radius:6px`. У коді діє новіший компонент `3147:10053` (гліф `18×18`,
`stroke-width:1.5`, light `#475569` / dark `#CBD5E1`) — він **заміщає** старий `×` Arial 20px
`#94A3B8` із цього фрейму. Це свідоме розходження, не помилка.

### 14.3 Stepper

`padding:16px 24px 0` · `gap:12px`. Крок: `flex:1 0 0` · `height:38px` ·
`border-bottom:2px` · `padding-bottom:14px` · по центру · `gap:8px`.
Кружечок номера: `22×22`, `radius:11px`, DM Sans **700 / 11px**.
Підпис: DM Sans **12px**.

| Стан | Тема | Лінія | Кружечок | Підпис |
|---|---|---|---|---|
| Active / Done | обидві | `#10B981` | фон `#10B981`, текст `#FFFFFF` | `600` · light `#475569` · dark `#F8FAFC` |
| Inactive | Light | `#E2E8F0` | фон `#FFFFFF`, бордер `#E2E8F0`, текст `#94A3B8` | `600` · `#94A3B8` |
| Inactive | Dark | `#04363A` | фон прозорий, бордер `rgba(255,255,255,0.2)`, текст `rgba(255,255,255,0.7)` | `500` · `#64748B` |

Stepper опційний: вікно без кроків просто його не рендерить.

### 14.4 Body (слот)

`padding:20px 0 10px` — **вертикальні відступи задає оболонка, горизонтальних немає**.
Кожен блок вмісту сам бере `padding:0 24px`. Так зроблено, щоб таблиця, смуга або роздільник
могли йти від краю до краю, не борючись із падингом контейнера.
Висота слота **не фіксується** — вона від вмісту; `overflow-y:auto` при переповненні.

⚠️ У макеті слот має конкретну висоту (`346px` light / `373px` dark) — це висота **інстансу**,
не правило. Не переносити її в код.

### 14.5 Footer

Обгортка `padding-top:8px`. Сам footer — `padding:10px 24px 12px`, `gap:8px`,
`border-top:1px` (light `#E2E8F0` · dark `rgba(255,255,255,0.06)`).
Розкладка — `space-between`: **Back (`Third`) ліворуч, Primary праворуч**.
Повний перелік дозволених композицій і значення кнопок — **§6.1**.

### 14.6 Відповідність коду (перевірено 18.08.2026)

Реалізація — `openModal()` / `.gm-card` (`index.html:4498`). Збігається з каноном:
ширина · radius · shadow · фони обох тем · header padding і типографіка тайтла ·
геометрія stepper і всі його стани · footer цілком.

Розходження, зафіксовані свідомо:

| Що | Канон | Код | Рішення |
|---|---|---|---|
| Close | `×` Arial 20px `#94A3B8` | компонент `3147:10053` | код новіший — лишаємо |
| Border оболонки | немає | `1px solid var(--border)` | лишаємо: без нього light-модалка зливається з підкладкою |
| `.gm-body` padding | `20px 0 10px` | `14px 24px 4px` | **розходження**; канон уже діє в `.gm-agentless-setup` / `.gm-agentless-testing` |
| `.gm-sub` колір | `--text-sec` | `--text-muted` | **розходження** |

Два останні рядки — єдине, що лишилось вирівняти. Міграція `.gm-body` зачіпає всі наявні
модалки (вміст доведеться загортати в `padding:0 24px`), тому робиться окремим кроком і лише
за явним рішенням.

### 14.7 Виняток — флов Add Manually не чіпати

**Rule (user, 18.08.2026).** Флов **Add Manually** — Add Node → detected OS / вибір OS → вибір
методу → Agentless Connect → Testing connection — доведений і працює як треба. Він **виключений
з будь-якої міграції оболонки**, зокрема з вирівнювання `.gm-body`.

Класи, яких це стосується: `.gm-add-node-wizard` · `.gm-add-node-choice` · `.gm-agentless-setup` ·
`.gm-agentless-testing` · `.gm-node-added`. Їхні власні override-и `.gm-head` / `.gm-stepper` /
`.gm-body` / `.gm-foot` — не «зайве дублювання», а зафіксована під Figma геометрія конкретних
екранів (див. `CLAUDE.md`, розділ Add Node modal flow). Не рефакторити й не підганяти під §14.4.

---

## 15. Progress bar

### 15.1 Кольори — ЄДИНЕ ДЖЕРЕЛО

**Source of truth:** Figma `eXz33Qu7v58JjOiatTptAE`, node **`3332:32980`** «Progress bar»
(варіанти `property1 = light | dark`, отримано 19.08.2026). Змінних до нод не привʼязано —
у макеті стоять літеральні hex, тому нижче саме вони.

**Rule.** Смуга прогресу має рівно **два кольори**: трек (незаповнена частина) і заливка.
Обидва **змінюються між темами**, геометрія — ні.

| Частина | Light | Dark |
|---|---|---|
| Трек (незаповнена частина) | `#E2E8F0` | `#335256` |
| Заливка | `#07986B` | `#09996C` |

Спільне для обох тем:

| Властивість | Значення |
|---|---|
| висота | `6px` |
| radius | `3px` (трек і заливка однаково) |
| overflow | `hidden` на треку |
| світіння заливки | `0 0 8px rgba(63,200,120,0.4)` |

⚠️ **Заливка в каноні — ПЛОСКИЙ колір, не градієнт.** Різниця light/dark мінімальна
(`#07986B` → `#09996C`), але вона є: у темній темі зелений трохи світліший, щоб компенсувати
темний трек. Не підставляти `var(--btn-grad)`: градієнт кнопок — це інший компонент.

⚠️ **Трек ніколи не бере `--border`, `--bg-subtle` чи `rgba(255,255,255,.08)`.** На картці
ці значення або зливаються з фоном (смуга читається як 100% заповнена), або дають інший відтінок
сірого, ніж сусідні смуги на тому самому екрані.

**Токени.** `--progress-track` і `--progress-fill` (`index.html:104` light, `:2923` dark).
Локальних кольорів смуги не задавати — жодних `--btn-grad`, `--border`, `--bg-page`, `--bg-subtle`.

**Reference.** Канонічний клас **`.pbar`** (`index.html:1326`): трек `<div class="pbar">`,
заливка — вкладений `<i>`. Ширина/анімація задаються на `<i>`, усе інше йде з класу.

### 15.2 Реалізація — зведено 19.08.2026

Було 6 незалежних родин смуг; тепер один блок і один набір значень. Легасі-імена лишились
**селекторами всередині канонічного блоку**, розмітку не переписували (та сама стратегія, що в §12).

| Родина | Де | Що лишилось власного |
|---|---|---|
| `.pbar > i` | Remediation rescan · Node Discovery (модалка) · `v-j3-syncing` · `v-j3-scanning` | — |
| `.gm-scan-track i` | AWS крок 5 «CIS Scan — In Progress» | `margin-top:10px` |
| `.cmdb-progress-track` / `-fill` | CMDB importing | `transition:width .3s` |
| `.scan-bar-wrap` / `-fill` | Onboarding scan · Add Manually scan | `margin-bottom:10px`, `animation:scanBar` |

Видалено як мертве або конфліктне: `.progress-wrap` / `.progress-row` / `.progress-track` /
`.progress-fill` (0 входжень у розмітці) · `body.dark .scan-bar-wrap` · `body.dark .cmdb-progress-track`
(обидва перебивали токен) · інлайн-фолбеки `var(--progress-track,#e2e8f0)`.

**Побічно виправлено:** смуга «~60% complete» на `v-j3-scanning` посилалась на неіснуючий keyframe
`scanBar60` і тому малювалась на 100%. Тепер статична `width:60%` — як у модальній версії.

**Exceptions.**
- `.progress-dots` (`2738`) — індикатор невизначеного очікування, не смуга прогресу.
- `.stat-bar-wrap` / `.stat-bar-fill` (`2241`) — **пропорція в KPI-картці**, а не прогрес
  (одна з них червона на 100% для «Critical Issues»). Колір несе значення, тому канон не діє.
- `.sidebar-footer-track` / `-fill` (`1039`) — відлік до наступного скану на **завжди темному**
  сайдбарі. Токен там не працює: у світлій темі `--progress-track` дав би `#E2E8F0` на темно-зеленому.
